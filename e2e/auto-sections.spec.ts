 ...
 * its heading, with the caret carrying on in the new section.
 */
test("start typing immediately; headings create sections automatically", async ({ page }) => {
  ...
  const firstEditor = page.getByRole("textbox", { name: "Section copy" }).first();
  ...
  await expect(page.getByRole("textbox", { name: "Section copy" })).toHaveCount(2, { timeout: 10_000 });
  await expect(page.getByLabel("Section title").nth(1)).toHaveValue("Our features");

  // the caret moved with the split — keep typing into the new section
  await page.keyboard.press("Enter");
  await page.keyboard.type("Fast, versioned, collaborative.");
  const secondEditor = page.getByRole("textbox", { name: "Section copy" }).nth(1);
  await expect(secondEditor).toContainText("Fast, versioned, collaborative.");
  // the first section kept only its own copy
  await expect(page.getByRole("textbox", { name: "Section copy" }).first()).not.toContainText("Our features");
  ...
  await page.reload();
  await expect(page.getByRole("textbox", { name: "Section copy" })).toHaveCount(2);
  await expect(page.getByLabel("Section title").nth(1)).toHaveValue("Our features");
  await expect(page.getByRole("textbox", { name: "Section copy" }).nth(1)).toContainText(
    "Fast, versioned, collaborative.",
  );
});
...
test("pasting multi-section copy splits it all at once", async ({ page }) => {
  ...
  await editor.click();
  // typed quickly, this is three sections' worth of copy
  ...
  await expect(page.getByRole("textbox", { name: "Section copy" })).toHaveCount(3, { timeout: 10_000 });
